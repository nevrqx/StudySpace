document.addEventListener('DOMContentLoaded', async () => {
    ...
    function updateStreak() {
        ...
        if (streakEl) {
            streakEl.innerHTML = `${streak} дн. <span style="color: #ff8c42;">🔥</span>`;
        ...
        if (filter === 'de') {
            vocabContainer.innerHTML = `
                <div class="empty-state-message" style="text-align: center; padding: 3rem 1rem; color: var(--text-secondary);">
                    ...
                    <h3>слов пока не добавлено но Лео это обязательно скоро сделает :)</h3>
                </div>
            `;
            return;
        }
    ...
    function toggleTextBookmark() {
        ...
        updateDropdownBookmarks(); // Sync dropdown
    }
    ...
    function updateDropdownBookmarks() {
        const bookmarks = JSON.parse(localStorage.getItem('bookmarkedTexts') || '[]');
        // Target specifically the English text selector dropdown items
        const dropdownItems = document.querySelectorAll('#english-text-selector .dropdown-item');
        ...
        dropdownItems.forEach(item => {
            ...
            if (bookmarks.includes(val)) {
                ...
                // Ensure icon exists
                if (!item.querySelector('.bookmark-icon-small')) {
                ...
                const icon = item.querySelector('.bookmark-icon-small');
                if (icon) icon.remove();
            }
        });
    }
    ...
    // --- Selection Reading Feature (PC Only) ---
    function initSelectionReading() {
        // Relaxed PC detection: valid if screen is wide enough (> 768px)
        // This covers laptops and desktops. Most mobiles are < 500px.
        const isPC = window.innerWidth > 768;
        if (!isPC) return;

        let selectionBtn = document.getElementById('selection-read-btn');
        if (!selectionBtn) {
            selectionBtn = document.createElement('button');
            selectionBtn.id = 'selection-read-btn';
            selectionBtn.className = 'selection-read-btn';
            selectionBtn.innerHTML = '<ion-icon name="volume-high"></ion-icon> Read';
        ...
        document.addEventListener('mouseup', (e) => {
            ...
            setTimeout(() => {
                ...
                if (text.length > 0 && !selection.isCollapsed) {
                    ...
                    let top = rect.top + scrollTop - 50; // 50px above
                    let left = rect.left + scrollLeft + (rect.width / 2) - (selectionBtn.offsetWidth / 2);

                    // Prevent going off-screen top
                    if (rect.top < 60) {
                        top = rect.bottom + scrollTop + 10; // Show below if too close to top
                    }
        ...
        // Hide on mousedown if not clicking the button itself
        document.addEventListener('mousedown', (e) => {
            if (e.target !== selectionBtn && !selectionBtn.contains(e.target)) {
                selectionBtn.classList.remove('visible');
            }
        });
    }
    ...
    function speakSelection(text) {
        ...
        // Use the existing voice selection logic if available
        const voice = getBestEnglishVoice();
        if (voice) {
            utterance.voice = voice;
            utterance.lang = voice.lang;
    ...
    function initFocusMode() {
        const isPC = window.innerWidth > 768; // Simple PC Check
        ...
        if (isPC) {
            ...
            focusBtn.addEventListener('click', () => {
                ...
                // Optional: Play a specific "mode switch" sound if available, otherwise click is enough
                if (isActive) {
                    showToast('Режим фокусировки включен', 'success');
                } else {
                    showToast('Режим фокусировки выключен', 'info');
                }
            });
    ...
    class SoundManager {
        ...
        initListeners() {
            // Refined Hover Logic: Use capture to handle all, but filter carefully
            let lastHover = 0;
            document.addEventListener('mouseover', (e) => {
                if (!this.enabled) return;

                const target = e.target.closest('button, .nav-item, .dropdown-item, .card-toolbar .icon-box');

                // 1. Ensure we actually hit a relevant target
                if (!target) return;

                // 2. Debounce time (fast mouse movements)
                const now = Date.now();
                if (now - lastHover < 50) return;

                // 3. Strict transition check: Ignore internal bubbles
                // Only play if we came from OUTSIDE this specific target
                const related = e.relatedTarget;
                if (target.contains(related)) return;

                // 4. Play sound (randomized slightly)
                this.playHover();
                lastHover = now;
            ...
            document.addEventListener('mousedown', (e) => {
                if (!this.enabled) return;
                const target = e.target.closest('button, .nav-item, .dropdown-item, .action-btn, #english-text-selector');
                if (target) {
                    ...
                    if (target.classList.contains('nav-item')) {
                        ...
                        this.playClick('mechanical');
                    } else {
                        this.playClick('standard');
                    }
                }
            });
        }
        ...
        playHover() {
            ...
            // Randomize pitch slightly for organic feel (+/- 50Hz)
            const randomDetune = (Math.random() - 0.5) * 50;
            const baseFreq = 800 + randomDetune;

            osc.frequency.setValueAtTime(baseFreq, t);
            osc.frequency.exponentialRampToValueAtTime(baseFreq + 400, t + 0.04);
        ...
        playClick(type = 'standard') {
            ...
            if (type === 'high') { // Nav items - crisp
                osc.type = 'sine';
                osc.frequency.setValueAtTime(600 + detune, t);
                osc.frequency.exponentialRampToValueAtTime(300, t + 0.15);
                gain.gain.setValueAtTime(0.05, t);
            } else if (type === 'soft') { // Dropdown - quiet
                osc.type = 'triangle';
                osc.frequency.setValueAtTime(400 + detune, t);
                osc.frequency.exponentialRampToValueAtTime(100, t + 0.1);
                gain.gain.setValueAtTime(0.03, t);
            } else if (type === 'mechanical') { // Action buttons - toggle click
                osc.type = 'square';
                // Low pass filter for "thud" might be too complex for simple osc, 
                // so we simulate with square/triangle mix or just envelope
                osc.frequency.setValueAtTime(200 + detune, t);
                osc.frequency.exponentialRampToValueAtTime(50, t + 0.08);
                gain.gain.setValueAtTime(0.02, t); // Square is loud, keep volume low
            } else { // Standard
                osc.type = 'sine';
                osc.frequency.setValueAtTime(350 + detune, t);
                osc.frequency.exponentialRampToValueAtTime(100, t + 0.1);
    ...
    function startReadingTimer(textId) {
        ...
        const timerDisplay = document.getElementById('timer-display');
        const timerContainer = document.getElementById('reading-timer');

        if (timerContainer) timerContainer.style.display = 'flex';

        updateReadStats(textId, 1, 0);

        readingTimerInterval = setInterval(() => {
            currentReadingSeconds++;
            if (timerDisplay) {
                const m = Math.floor(currentReadingSeconds / 60).toString().padStart(2, '0');
                const s = (currentReadingSeconds % 60).toString().padStart(2, '0');
                timerDisplay.textContent = `${m}:${s}`;
            }
            if (currentReadingSeconds % 10 === 0) {
                updateReadStats(textId, 0, 10);
            }
        }, 1000);
    }
    ...
    function updateReadStats(textId, incrementCount = 0, incrementTime = 0) {
        const stats = JSON.parse(localStorage.getItem('readingStats') || '{}');
        if (!stats[textId]) {
            stats[textId] = { reads: 0, time: 0 };
        }

        stats[textId].reads += incrementCount;
        stats[textId].time += incrementTime;

        localStorage.setItem('readingStats', JSON.stringify(stats));

        const countDisplay = document.getElementById('read-count-display');
        if (countDisplay) {
            countDisplay.innerText = `${stats[textId].reads} чтений`;
        }
    }

    // Ensure bookmarks are visually updated on load (redundant check)
    if (typeof updateDropdownBookmarks === 'function') {
        updateDropdownBookmarks();
    }
